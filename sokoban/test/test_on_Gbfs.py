...
class Gbfs:
    def __init__(self, dim, loss):
        ...
        self._loss = loss
        self._nn = NN(dim)
        self._nn.model.load_weights("sok_gbfs_" + loss)
    ...
    def find_goal_state(self, goal_state, box_tar):
        ...
        return (to_categorical_tensor(goal_state, box_tar, self._dim, self._dim))
    ...
    def findNN(self, stateName, box_tar, goal_state):  # Strips state
        ...
        val = self._nn.model.predict([state.reshape(1, self._dim, self._dim, 5), goal_state.reshape(1, self._dim, self._dim, 5)],verbose = 0)[1][0][0]
