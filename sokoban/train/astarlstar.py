...
def findNN(stateName, box_tar, goal_state):#Strips state
    ...
    val = nn.model.predict([state.reshape(1,dim,dim,5),goal_state.reshape(1,dim,dim,5)])[1][0][0]
...
def Astar_lstar(init,box_tar):
    ...
    while True:
        ...
        if check_goal(stateName, box_tar):
            ...
            X_Train, Y_Train = create_one_hot(key_states,box_tar,goal_state)
...
def create_one_hot(key_states, box_tar, goal_state):
    X_Train = []
    Y_Train = []
    for k in key_states:
