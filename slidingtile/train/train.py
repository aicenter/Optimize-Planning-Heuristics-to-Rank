...
def train_minibatches(x_train,y_train,train_loss,search_alg,h_matrix=None,path_cost=None,h_val = None,child_states_x=None,child_states_y = None):
    ...
    nn.model.save_weights("stp_"+search_alg+"_"+train_loss)
...
def create_minibatches_astar(coll_states,train_loss,search_alg):
    ...
    for i in range(20000):
        ...
        if train_loss == "lstar":
            X_Train, Y_Train, h_matrix, path_cost = Astar_lstar(state)
            train_minibatches(X_Train,Y_Train, train_loss, search_alg, h_matrix=h_matrix, path_cost=path_cost)
        if train_loss == "lgbfs":
            X_Train, Y_Train, h_matrix = Astar_lgbfs(state)
            train_minibatches(X_Train,Y_Train,train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "lrt":
            X_Train, Y_Train, h_matrix = Astar_lrt(state)
            train_minibatches(X_Train,Y_Train,train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "l2":
            X_Train, Y_Train, h_val = Astar_l2(state)
            train_minibatches(X_Train,Y_Train,train_loss, search_alg, h_val=h_val)
        if train_loss == "bellman":
            X_Train, Y_Train, child_states_x, child_states_y, h_val = Astar_bell(state)
...
def create_minibatches_gbfs(coll_states,train_loss,search_alg):
    ...
    for i in range(20000):
        ...
        if train_loss == "lstar":
            X_Train, Y_Train, h_matrix, path_cost = Gbfs_lstar(state)
            train_minibatches(X_Train,Y_Train, train_loss, search_alg, h_matrix=h_matrix, path_cost=path_cost)
        if train_loss == "lgbfs":
            X_Train, Y_Train, h_matrix = Gbfs_lgbfs(state)
            train_minibatches(X_Train,Y_Train,train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "lrt":
            X_Train, Y_Train, h_matrix = Gbfs_lrt(state)
            train_minibatches(X_Train,Y_Train,train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "l2":
            X_Train, Y_Train, h_val = Gbfs_l2(state)
            train_minibatches(X_Train,Y_Train,train_loss, search_alg, h_val=h_val)
        if train_loss == "bellman":
            X_Train, Y_Train, child_states_x, child_states_y, h_val = Gbfs_bell(state)
            train_minibatches(X_Train,Y_Train,train_loss,search_alg,child_states_x=child_states_x,child_states_y=child_states_y,h_val=h_val)
