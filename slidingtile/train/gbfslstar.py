...
dim = 5 # dimensions set to 10*10 during training
...
def findNN(stateName, goal_state):#Strips state
    ...
    val = nn.model.predict([state.reshape(1,dim,dim,25),goal_state.reshape(1,dim,dim,25)])[1][0][0]
    return val
...
def Gbfs_lstar(init):
    goal_state, _ = np.array(find_goal_state())
    start = time.time()
    ...
    while True:
        ...
        if check_goal(stateName):
            ...
            path_cost = find_pathcost(key_states)

            X_Train, Y_Train = create_one_hot(key_states,goal_state)
            return X_Train, Y_Train, h_matrix, path_cost
            break
...
def find_constraints(key_states):
    ...
    for i in range(len(key_states)):
        for j in range(i+1,len(key_states)):
            if (G.nodes[key_states[i]]['o'] == 1 and G.nodes[key_states[j]]['o'] == 0) or (G.nodes[key_states[i]]['o'] == 0 and G.nodes[key_states[j]]['o'] == 1):
                    constraints+=1
...
def find_pathcost(key_states):
    path_cost = []
    for k in range(len(key_states)):
        path_cost.append(G.nodes[key_states[k]]['g'])
    return path_cost

def create_one_hot(key_states, goal_state):
    X_Train = []
    Y_Train = []
    for k in key_states:
