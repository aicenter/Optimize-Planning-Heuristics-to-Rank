...
class Astar:
    def __init__(self, dim, loss):
        ...
        self._loss = loss
        self._nn = NN(dim)
        self._nn.model.load_weights("stp_astar_"+loss)
        if dim == 5:
    ...
    def findNN(self, stateName, goal_state):  # Strips state
        ...
        val = self._nn.model.predict([state.reshape(1, self._dim, self._dim, 25), goal_state.reshape(1, self._dim, self._dim, 25)])[1][0][0]
        return val
