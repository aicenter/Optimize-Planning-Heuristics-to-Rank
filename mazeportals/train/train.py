...
from gbfsastar import Gbfs_lstar
...
def bellman_loss(pred_cost,true_cost,child_states_x):
    ...
    for i in range(len(child_states_x)):
        h+= tf.maximum(0,1+tf.reduce_min(nn.model(child_states_x[i]))- pred_cost[i])
...
def train_minibatches(x_train,train_loss,search_alg,h_matrix=None,path_cost=None,h_val = None,child_states_x=None):
    optimizer = tf.keras.optimizers.Adam()
    with tf.GradientTape() as tape:
        logits = nn.model(x_train, training=True)
        if train_loss == "lstar":
        ...
        if train_loss == 'bellman':
            loss_value = bellman_loss(logits,h_val,child_states_x)
    grads = tape.gradient(loss_value, nn.model.trainable_weights)
    optimizer.apply_gradients(zip(grads, nn.model.trainable_weights))
    nn.model.save_weights("maze_"+search_alg+"_"+train_loss)

def create_minibatches_astar(coll_states,train_loss,search_alg):
    #Train for 20000 times
    for i in range(20000):
        state = coll_states[random.randrange(len(coll_states))]
        #print(state)
        if train_loss == "lstar":
            X_Train, h_matrix, path_cost = Astar_lstar(state)
            train_minibatches(X_Train, train_loss, search_alg, h_matrix=h_matrix, path_cost=path_cost)
        if train_loss == "lgbfs":
            X_Train, h_matrix = Astar_lgbfs(state)
            train_minibatches(X_Train, train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "lrt":
            X_Train, h_matrix = Astar_lrt(state)
            train_minibatches(X_Train,train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "l2":
            X_Train, h_val = Astar_l2(state)
            train_minibatches(X_Train, train_loss, search_alg, h_val=h_val)
        if train_loss == "bellman":
            X_Train, child_states_x, h_val = Astar_bell(state)
            train_minibatches(X_Train,train_loss,search_alg,child_states_x=child_states_x,h_val=h_val)
...
def create_minibatches_gbfs(coll_states,train_loss,search_alg):
    ...
    for i in range(20000):
        state = coll_states[random.randrange(len(coll_states))]
        if train_loss == "lstar":
            X_Train, h_matrix, path_cost = Gbfs_lstar(state)
            train_minibatches(X_Train, train_loss, search_alg, h_matrix=h_matrix, path_cost=path_cost)
        if train_loss == "lgbfs":
            X_Train, h_matrix = Gbfs_lgbfs(state)
            train_minibatches(X_Train, train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "lrt":
            X_Train, h_matrix = Gbfs_lrt(state)
            train_minibatches(X_Train,train_loss, search_alg, h_matrix=h_matrix)
        if train_loss == "l2":
            X_Train, h_val = Gbfs_l2(state)
            train_minibatches(X_Train,train_loss,search_alg,h_val=h_val)
        if train_loss == "bellman":
            X_Train, child_states_x, h_val = Gbfs_bell(state)
            train_minibatches(X_Train,train_loss,search_alg,child_states_x=child_states_x,h_val=h_val)
